...
insert into public.app_config (key, value, scope, description) values
  ...
  ('notifications.push_enabled',      'false'::jsonb,                       'server', 'Livraison push réelle (staging : true + send_push_url + secret)'),
  ('notifications.send_push_url',     '""'::jsonb,                          'server', 'URL de la fonction send-push (staging)'),
