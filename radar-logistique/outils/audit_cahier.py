...
REGLES = [
    ...
    ("l'absence d'information n'est jamais un avantage",
     ...
     ["une_annonce_muette_ne_bat_pas_une_annonce_couverte",
      ...
      "deux_excellentes_opportunites_restent_departageables"]),
    ...
    ("le radar n'est pas un moteur d'appels d'offres",
     ["chaine.traiter", "rapport.bloc_de"],
     ["le_radar_produit_un_resultat_commercial_complet",
      "le_pipeline_commercial_est_alimente_sans_aucun_avis",
      "aucune_procedure_n_est_inventee",
      "les_signaux_ne_deviennent_pas_des_contrats"]),
    ("un besoin privé fort bat un marché public faible",
     ["score.Bareme._taille"],
     ["le_prive_a_12k_bat_le_public_a_8k",
      "la_sous_traitance_a_15k_bat_le_public_a_8k",
      "memes_donnees_economiques_sources_differentes_meme_score",
      "source_identique_economies_differentes_scores_differents"]),
    ("aucune source n'est indispensable, TED comprise",
     ["chaine.traiter"],
     ["retirer_ted_ne_change_rien_aux_autres",
      "chaque_adaptateur_peut_etre_retire",
      "le_coeur_ne_nomme_aucun_adaptateur"]),
    ("le rapport est un pipeline commercial en huit blocs",
     ["rapport.BLOCS_PIPELINE", "rapport.Rapport._pipeline"],
     ["le_pipeline_commercial_est_alimente_sans_aucun_avis"]),
    ("le score reste départageable jusqu'en haut",
     ["score.Bareme.calculer"],
     ["deux_excellentes_opportunites_restent_departageables",
      "le_plafond_ne_s_atteint_que_sur_un_cas_parfait"]),
    ("le CPV est une preuve, jamais une autorité",
     ["role.DetecteurDeRole.analyser"],
     ["un_cpv_de_fourniture_n_ecrase_pas_un_texte_de_prestation",
      "le_prive_atteint_le_meme_niveau_de_reconnaissance_que_le_public",
      "une_contradiction_ne_produit_jamais_un_rejet"]),
    ("le produit n'est pas un radar d'appels d'offres",
     ["chaine.traiter", "nature.EVENEMENT_OBSERVABLE"],
     ["la_chaine_complete_fonctionne_sur_ce_corpus",
      "aucune_de_ces_opportunites_n_invente_de_procedure",
      "aucun_montant_n_est_fabrique",
      "le_rapport_produit_un_pipeline_commercial_complet"]),
    ("un signal reste un signal, quelle que soit la source",
     ["nature.qualifier", "classification.classer"],
    ...
    # §VALEUR — le radar cherche de l'argent, pas des documents.
    ("adéquation et potentiel ne sont jamais écrasés en un seul nombre",
     ["priorite.evaluer", "priorite.Priorite"],
     ["le_score_sature_au_dela_du_confortable",
      "deux_affaires_tres_differentes_ont_le_meme_score",
      "le_potentiel_lui_distingue_ces_deux_affaires",
      "le_classement_commercial_suit_le_CA_pas_le_score"]),
    ("grosse affaire ≠ grosse affaire adaptée",
     ["priorite.Priorite", "capacite.Bilan.plan_de_faisabilite"],
     ["l_intensite_distingue_deux_affaires_de_meme_CA",
      "une_petite_affaire_dense_bat_une_grosse_affaire_maigre_en_intensite",
      "la_marge_reste_NON_MESUREE_sans_couts_au_profil"]),
    ("un avis publié se postule, quelle que soit sa forme",
     ["nature.qualifier", "classification.classer"],
     ["un_avis_sans_echeance_publiee_reste_un_FAIT",
      "et_donc_se_postule",
      "les_deux_etages_lisent_le_type_de_la_meme_facon",
      "une_page_privee_nue_reste_une_HYPOTHESE"]),
    ("une opportunité non chiffrée reste exploitable sans être valorisée",
     ["chiffre_affaires.mesurer", "priorite.Priorite"],
     ["elle_nest_pas_rejetee", "son_CA_est_NON_PUBLIE_et_son_potentiel_NON_MESURABLE",
      "elle_porte_une_question_explicite",
      "elle_ne_passe_pas_devant_une_affaire_chiffree"]),
    ("détecter n'est pas valoriser",
     ["priorite.Couverture"],
    ...
    ("AUCUNE SOURCE NE PEUT ÊTRE LE MODÈLE IMPLICITE DU PRODUIT",
     ...
     ["les_seize_detecteurs_passent",
      "le_meme_besoin_sous_huit_formes_vaut_le_meme_score",
      "le_coeur_ne_recoit_meme_plus_la_provenance"]),
    ("postuler ou appeler ne dépend pas du secteur de l'acheteur",
     ["classification.classer", "procedure.Lecture.depot_organise"],
     ["une_entreprise_privee_qui_consulte_se_postule",
      "un_acheteur_public_sans_procedure_se_contacte",
      "une_date_de_validite_ne_fait_pas_un_guichet_de_depot",
      "une_url_de_page_nest_pas_un_guichet_de_depot"]),
    ("le radar fonctionne sans aucun marché public",
     ["chaine.traiter", "rapport.construire"],
     ["la_chaine_va_jusqu_au_bout",
      "le_rapport_est_complet_sans_le_moindre_marche_public",
      "la_deduplication_fonctionne_sans_reference_officielle",
      "le_suivi_fonctionne_sans_procedure"]),
    ("les marchés publics ne sont qu'un capteur",
     ["chaine.Moteur.analyser"],
     ["ajouter_les_marches_publics_ne_change_pas_les_besoins_prives",
      "retirer_les_adaptateurs_publics_laisse_un_radar_entier"]),
    ("la complétude est mesurée avec la grille du besoin",
     ["rapport.CHAMPS_PAR_FAMILLE"],
     ["un_signal_n_est_pas_mesure_sur_lots_et_echeance",
      "un_besoin_prive_est_complet_sur_sa_propre_grille",
      "un_marche_public_garde_sa_grille_complete"]),
    ("le corpus de formulations ne régresse pas",
     ["procedure.interpreter_formulation", "procedure.MARQUEURS"],
     ["aucune_formulation_du_corpus_n_est_mal_comprise",
      "offres_non_recevables_n_est_jamais_postulable",
      "le_corpus_ne_declare_aucune_donnee_reelle"]),
    ("un fait vérifiable bat une étiquette de rangement",
     ["procedure.lire"],
     ["une_date_depassee_bat_la_rubrique_du_portail",
      "une_attribution_annoncee_ne_laisse_pas_postuler"]),
    ("une donnée contradictoire est dite illisible, pas interprétée",
     ["chaine.Moteur.analyser", "construction.evaluer"],
     ["une_echeance_posterieure_au_demarrage_est_illisible"]),
    ("cent opportunités mélangées retrouvent leur famille",
     ["rapport.famille_de"],
     ["les_cent_opportunites_melangees_retrouvent_leur_famille",
      "les_quatre_lots_gardent_quatre_etats_distincts"]),
