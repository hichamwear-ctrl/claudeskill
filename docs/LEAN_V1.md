...
| **M12** ✅ | **intégration staging & prod-ready** | Realtime Postgres Changes (publication) ; storage `mission-proofs` (participants) ; livraison push `notifications→send-push→Expo` (gardée `app_config`) ; `delivered_at` ; `docs/DEPLOYMENT.md` (secrets + checklist) ; **0 fonctionnalité métier** |
| **M13+** | config versioning, litiges, ratings… | `config_*`, `disputes`, `ratings` |
