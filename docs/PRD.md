...
| **P10** | **Aucune dette d'architecture** | Chaque étape respecte l'**architecture cible**, reste **compatible avec les fonctionnalités différées**, évite les **migrations complexes** futures et les **hacks temporaires**, et demeure **additive** (on ajoute, on ne casse pas). Si un doute : on **revoit la conception avant de coder**. Distinction permanente **implémenté V1** vs **cible différée** (`LEAN_V1.md`). |
| **P11** | **Le parcours ne dépend jamais d'une catégorie** | Même si la classification échoue ou renvoie `unknown`, le moteur **continue** : questions génériques → récapitulatif → `pending_review`. L'opérateur peut accepter une demande **totalement inédite**. **L'absence de catégorie n'empêche jamais** un client d'envoyer une demande. La taxonomie **optimise** (questions, prix, workflow, stats, suggestions) ; elle ne **limite jamais**. *« Nous ne créons pas des services, nous traitons des demandes. »* |
| **P12** | **Toutes les Edge Functions restent génériques** | **Interdit** : `create_plumber_request`, `create_locksmith_request`, `book_*`… Le backend est bâti sur des fonctions **génériques** (`converse`, `review`, `payments`, `submit_request`, `transition_mission`, `estimate_price`, `zone_check`, `send_push`). Ajouter un métier = **enrichir les données** (classification, questions, workflow, tarif, règles, textes), **jamais** écrire de logique métier. |
